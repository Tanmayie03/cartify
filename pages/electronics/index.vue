<template>
  <div>
    ...
    <div
      ...
      <span
        v-for="p in electronicsection.filter(filterProducts)"
        class="transform hover:shadow-2xl"
        :key="index">
        <div class="flex flex-col justify-between p-4 text-left">
          <NuxtLink :to="`/electronics/${p.id}`">
            <div class="flex justify-center overflow-hidden">
              <img :src="p.image" class="w-[220px] h-[280px]" /></div
          ></NuxtLink>
          ...
          <div class="flex flex-col my-4">
            <NuxtLink :to="`/electronics/${p.id}`"
              ><p
                class="font-semibold truncate lg:text-lg text-slate-500 hover:text-slate-800">
                {{ p.title }}
              </p>
            </NuxtLink>
            <!-- <p class="mx-4 text-lg font-semibold">{{ category }}</p>  -->
            <!-- <p class="w-[70%] mx-4 mt-4 text-lg text-justify">{{ p.description }}</p> -->
          </div>
          <div class="flex text-center">
            <p class="text-xl font-semibold text-left text-green-600">
              ₹{{ p.price }}
            </p>
            <p class="mx-4 mt-1 text-sm text-green-600">Special Price</p>
          </div>
...
let electronicsection = [];
if (newprod && Array.isArray(newprod)) {
  electronicsection = newprod.filter(
