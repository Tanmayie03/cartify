<template>
  <div>
    <div
      ...
      <span
        v-for="p in jewelsection.filter(filterProducts)"
        class="transform hover:shadow-2xl"
        :key="index">
        <div class="flex flex-col justify-between p-4 text-left">
          <NuxtLink :to="`/jewel/${p.id}`">
            <div class="flex justify-center overflow-hidden">
              <img :src="p.image" class="w-[fit] h-[280px]" /></div
          ></NuxtLink>
...
if (newprod && Array.isArray(newprod)) {
  jewelsection = newprod.filter((jewel) => jewel.category === "jewelery");
}
